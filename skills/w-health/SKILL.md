...
if [ ! -f "${HEALTH_SCRIPT:-}" ]; then
  for candidate in \
    "./skills/w-health/scripts/collect-data.sh" \
    "$(npx skills path tw93/Waza 2>/dev/null)/skills/w-health/scripts/collect-data.sh"; do
...
```bash
bash "$(dirname "$HEALTH_SCRIPT")/w-check-agent-context.sh" . summary
```
...
```bash
bash "$(dirname "$HEALTH_SCRIPT")/w-check-maintainability.sh" . summary
```

For deep audits:

```bash
bash "$(dirname "$HEALTH_SCRIPT")/w-check-maintainability.sh" . deep
```
...
```bash
bash "$(dirname "$HEALTH_SCRIPT")/w-check-doc-refs.sh" .
```
...
```bash
bash "$(dirname "$HEALTH_SCRIPT")/w-check-verifier-output.sh" . <log-file>
```
...
- Never act as a heavy lint, typecheck, duplication, or architecture-rewrite substitute; `/w-health` reports maintainability guardrails and concrete next actions only.
...
| `/w-health` burned too much quota on first run | Stay in summary mode first. Full conversation extracts and inspector subagents are deep-audit tools, not the default path for Standard projects. |
