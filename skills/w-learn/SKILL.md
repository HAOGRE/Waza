...
**Boundary**: single URL that only needs fetching belongs in `/w-read`. A single URL that needs summary or analysis can use `/w-read` as the fetch step, but the final answer should satisfy the user's requested summary or analysis. `/w-learn` is for multi-source research that produces a new structured output.

## Pre-check

Check whether `/w-read` and `/w-write` skills are installed (look for their SKILL.md in the skills directories). Warn if missing, do not block:
- `/w-read` missing -- Phase 1 fetch falls back to native `WebFetch` / `curl`; coverage on paywalled, JS-heavy, and Chinese-platform pages degrades.
- `/w-write` missing -- Phase 5 AI-pattern stripping falls back to manual scan. Phases 1-4 are unaffected.
...
2. **Fetch** -- every URL goes through `/w-read` when available. `/w-read` owns the proxy cascade, paywall detection, and platform routing (WeChat, Feishu, PDF, GitHub). Native fetch tools and raw `curl` silently fail on JS-heavy or paywalled sites and skip all of that. If `/w-read` is missing (Pre-check warned), fall back to native fetch and accept reduced coverage.
3. **File** -- tell `/w-read` the research project's source directory when one exists. If no directory was specified, let `/w-read` use a per-session temp directory and return the saved path. Move or index saved files into sub-topic directories after fetch returns. Move, don't refetch.
...
| Used native fetch tools or `curl` on URLs while `/w-read` was installed | Phase 1 fetch is not optional. `/w-read` owns the proxy cascade, paywall detection, and platform routing. Bypassing it silently loses coverage on paywalled, JS-heavy, or Chinese-platform pages. |
