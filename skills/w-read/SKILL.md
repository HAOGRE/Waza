...
- "convert", "fetch as Markdown", "原文", "全文", "quote", "cite", "save", "下载", and `/w-learn` calls: return or save clean Markdown.
...
After routing, load `references/w-read-methods.md` and run the commands for the chosen method.
...
- Called from within `/w-learn` (Phase 1 expects a file path to organize)
- User says "save" or "保存" after seeing the output (use conversation content, do not re-fetch)

When saving:
- Prefer the directory named by the user or by `/w-learn`. If none is provided, create a per-session temp directory and report its full path.
...
| Local fallback tools returned JSON | Extract the Markdown-bearing field. Raw JSON is not a valid final output for `/w-read`. |
