...
**Update check (non-blocking).** Once per conversation, run `bash <skill-base-dir>/scripts/w-check-update.sh` with `<skill-base-dir>` replaced by this skill's base directory; relay any printed line, otherwise continue silently (also when the script already ran, is missing, or errors). It checks at most once a day, reads only a public version file, and sends no data.
...
3. **Language detected from the text being edited**, not the user's command:
   - Contains Chinese characters + release notes or social post mode → load `references/w-write-zh-release-notes.md`
   - Contains Chinese characters + bilingual or translation review → load `references/w-write-zh-bilingual.md`
   - Product/site/app localization review across multiple locales → load `references/w-write-product-localization.md`; also load `references/w-write-zh-bilingual.md` when Chinese copy is present
   - Contains Chinese characters (default prose) → load `references/w-write-zh-prose.md` (quick rules); load `references/w-write-zh.md` for the full AI-taste pattern catalog
   - Otherwise → load `references/w-write-en.md`
...
For `/w-write`: the supplied text and current release state override memory. Durable preferences can set brevity, tone, and social-post shape; they do not override the hard rule to edit in place, keep meaning intact, and avoid change lists unless the user explicitly asks.
...
```bash
GATE="${CLAUDE_SKILL_DIR:+$CLAUDE_SKILL_DIR/scripts/w-check-punctuation.sh}"
[ -f "${GATE:-}" ] || GATE="${CLAUDE_SKILL_DIR:+$CLAUDE_SKILL_DIR/skills/w-write/scripts/w-check-punctuation.sh}"
[ -f "${GATE:-}" ] || GATE="./skills/w-write/scripts/w-check-punctuation.sh"
[ -f "${GATE:-}" ] || GATE="$(npx skills path tw93/Waza 2>/dev/null)/skills/w-write/scripts/w-check-punctuation.sh"
...
`${CLAUDE_SKILL_DIR}` is host-injected. The first path is this skill's own `scripts/` (standalone skill, full bundle, or repo); the fallbacks cover the inlined-root release ZIP, where the script ships under `skills/w-write/scripts/`.
...
2. **Propose cuts as change-points.** Show before to after for each structural cut or merge and let the user pick the subset. Never delete a whole section or paragraph silently; confirm first, since it may hold a fact found nowhere else (see `references/w-write-zh.md` 删段之前先确认信息量).
3. **Then line-level de-AI**, section by section, per `references/w-write-zh.md`.
...
Load `references/w-write-product-localization.md`. If Chinese is one of the locales, also load `references/w-write-zh-bilingual.md`.
