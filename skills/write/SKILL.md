...
It flags half-width punctuation inside CJK sentences, full-width spaces (U+3000), missing or wrong CJK/Latin spacing, em/en dashes, and locale punctuation mixing. It skips code fences, inline code, URLs, and the target of an inline markdown link; the visible `[label]` is still checked. Fix every finding while preserving meaning; `--fix` auto-applies the zero-ambiguity zh fixes to stdout. Note: `--lang auto` picks one language for the whole input by scanning in fixed priority: any kana routes to ja, else any CJK to zh, else any Hangul to ko (reserved, skipped), else en. A mostly-Chinese text that merely quotes a Korean glyph still routes to zh; pass an explicit `--lang` for mixed-locale or predominantly-English text. The checker is the source of truth for the character-level punctuation it covers; it does NOT check quote direction or other judgment calls, which stay with you and the reference files. This gate only protects text that flows through `write`.
